...
#define dyco_WEBSOCKET_SERVER_PORT	9096
...
struct _dyco_websocket_head_126 {
	unsigned short payload_length;
	char mask_key[4];
	unsigned char data[8];
} __attribute__ ((packed));

struct _dyco_websocket_head_127 {
...
typedef struct _dyco_websocket_head_127 dyco_websocket_head_127;
typedef struct _dyco_websocket_head_126 dyco_websocket_head_126;
typedef struct _dyco_ophdr dyco_ophdr;


static int dySetNonblock(int fd) {
	int flags;
...
char* decode_packet(unsigned char *stream, char *mask, int length, int *ret) {
	...
	dyco_ophdr *hdr =  (dyco_ophdr*)stream;
	unsigned char *data = stream + sizeof(dyco_ophdr);
	int size = 0;
	int start = 0;
	//char mask[4] = {0};
	int i = 0;
	...
	if ((hdr->mask & 0x7F) == 126) {
		...
		dyco_websocket_head_126 *hdr126 = (dyco_websocket_head_126*)data;
		...
		dyco_websocket_head_127 *hdr127 = (dyco_websocket_head_127*)data;
...
int encode_packet(char *buffer,char *mask, char *stream, int length) {
	...
	if (length < 126) {
		...
		memcpy(buffer, &head, sizeof(dyco_ophdr));
		size = 2;
	} else if (length < 0xffff) {
		dyco_websocket_head_126 hdr = {0};
		hdr.payload_length = length;
		memcpy(hdr.mask_key, mask, 4);

		memcpy(buffer, &head, sizeof(dyco_ophdr));
		memcpy(buffer+sizeof(dyco_ophdr), &hdr, sizeof(dyco_websocket_head_126));
		size = sizeof(dyco_websocket_head_126);
		
	} else {
		
		dyco_websocket_head_127 hdr = {0};
		hdr.payload_length = length;
		memcpy(hdr.mask_key, mask, 4);
		
		memcpy(buffer, &head, sizeof(dyco_ophdr));
		memcpy(buffer+sizeof(dyco_ophdr), &hdr, sizeof(dyco_websocket_head_127));

		size = sizeof(dyco_websocket_head_127);
...
void server_reader(void *arg) {
	...
	while (1) {
		...
		length = dyco_recv(fd, stream, MAX_BUFFER_LENGTH, 0);
		if (length > 0) {
...
int init_server(void) {
	...
	addr.sin_port = htons(dyco_WEBSOCKET_SERVER_PORT);
...
void server(void *arg) {
	...
	while (1) {
		...
		int cli_fd = dyco_accept(sockfd, (struct sockaddr*)&remote, &len);
		if (cli_fd % 1000 == 999) {
