...
#define SERVER_STRING "Server: dyco_httpd/0.1.0\r\n"
#define STDIN   0
#define STDOUT  1
#define STDERR  2

#define ENABLE_DYCO	0

#if ENABLE_DYCO

#define socket 	dyco_socket 
#define accept 	dyco_accept 
#define recv(a, b, c, d) 	dyco_recv(a, b, c, d)
#define send(a, b, c, d) 	dyco_send(a, b, c, d) 
...
void accept_request(void* arg)
{
	...
	while (1) {
		...
		numchars = dyco_recv(client, buf, sizeof(buf), 0);
		if (numchars > 0) {
			serve_file(client);
		} else if (numchars == 0) {	
			dyco_close(client);
...
void headers(int client)
{
	...
	int ret = dyco_send(client, buf, strlen(buf), 0);
    if (ret == -1) {
        printf("send : errno : %d\n", errno);
    } else {
...
void server(void *arg) {
	...
	while (1) {
		...
		int clientfd = dyco_accept(fd, (struct sockaddr*)&client_addr, &client_len);
		unlock();
...
int main(int argc, char *argv[]) {
	...
	if (pid > 0) {
		printf("dyco_httpd server running ...\n");
		getchar();
	} else if (pid == 0) {
		process_bind(fd);
	}
